    ...
    try:
        ...
        if result.get('status') == 'success':
            ...
            if result.get('root_service') != 'unknown':
                print(f"   ✅ 根因服务识别成功（修正生效）")
                print(f"   🎯 识别出的根因服务: {result.get('root_service')}")
            else:
                print(f"   ⚠️ 根因服务仍为 unknown（需要进一步调试）")
            
            if result.get('root_reason') != 'no_evidence':
                print(f"   ✅ 根因原因识别成功（修正生效）")
                print(f"   🎯 识别出的根因原因: {result.get('root_reason')}")
            else:
                print(f"   ⚠️ 根因原因仍为 no_evidence（需要进一步调试）")
            
            if result.get('confidence', 0.0) > 0.0:
                print(f"   ✅ 置信度大于 0（修正生效）")
                print(f"   🎯 置信度: {result.get('confidence', 0.0):.3f}")
            else:
                print(f"   ⚠️ 置信度仍为 0（需要进一步调试）")
...
def main():
    ...
    try:
        # 测试修正后的打分功能
        scoring_success = test_fixed_scoring()
        ...
        # 输出结果
        print(f"\n📊 测试结果:")
        print(f"   修正后打分: {'✅ 通过' if scoring_success else '❌ 失败'}")
        
        print(f"\n📈 总体结果: {'✅ 通过' if scoring_success else '❌ 失败'}")
