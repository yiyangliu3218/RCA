...
import json,csv
from ..timeutil import to_aware_utc, parse_any_ts_utc, parse_opencra_timestamp
import pandas as pd
        ...
        with open(path, "r") as f:
            ...
            for row in reader:
                yield {
                    ...
                    "startTime": parse_opencra_timestamp(int(row["timestamp"]) / 1000),
...
def iter_openrca_spans(path: str,window):
    df = pd.read_csv(path)
    trace_df = df[(df['timestamp'] / 1000 >= window.get("start")) &
                      (df['timestamp'] / 1000 <= window.get("end"))]
    ...
    spans = [
        {
            "traceId": row["trace_id"],
            "spanId": row["span_id"],
            "parentSpanId": row["parent_id"] if pd.notna(row["parent_id"]) else None,
            "service": row["cmdb_id"],
            "operation": "unknown_operation",
            "startTime": int(row["timestamp"]) // 1000,
            "duration": int(row["duration"]),
        }
        for row in records
    ]
